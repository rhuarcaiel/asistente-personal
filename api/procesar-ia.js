        ...
        try {
            ...
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `Eres un asistente de calendario experto. La fecha de hoy es ${today}. El usuario te pide que hagas algo. Analiza su petición y determina si es para CREAR un evento o ELIMINAR uno. Devuelve ÚNICAMENTE un objeto JSON con esta estructura: { "intent": ("create" o "delete"), "summary": (string), "start_datetime": (string, YYYY-MM-DDTHH:MM:SS), "is_recurring": (booleano), "recurrence": (objeto o null, igual que antes), "timezone": (string, "Europe/Madrid") }. Si es para eliminar, intenta encontrar la fecha y hora exactas que menciona el usuario.`
                    },
        ...
        if (proposal.intent === 'create') {
            ...
            try {
                ...
                if (!deleteResponse.ok) {
                    return res.status(500).json({ success: false, error: 'No se pudo eliminar el evento.' });
                }
